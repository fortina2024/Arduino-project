...
String server = "https://mumipoc.komatel.com/api/passagers";//"https://20.8.56.118/api/passagers";
...
String readRFID() {
  ...
  for (int i = 0; i < sizeof(blocksToRead); i++) {
    ...
    if (status == MFRC522::STATUS_OK) {
      ...
      Serial.print(blockTitres[i]);
      Serial.println(blockContent); 

      lcd.print(blockTitres[i]);
      lcd.setCursor(0, 1);
...
String readGPS() {
  String gps_Data = "";
  //Serial.println("Envoi de la commande AT+CGNSINF");
...
void sendDataToServer(String rfid, String gps) {
  rfid=1;
  String postData = "id_rfid=" + rfid + "&" + gps + "&id_agent=1&id_bus=1";
